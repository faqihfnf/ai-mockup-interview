...
function Feedback({ params }) {
  ...
  return (
    <div className="p-5">
      <h2 className="text-5xl font-bold text-green-600 text-center mt-20">
        Selamat Anda Telah Selesai!
      </h2>
      <h2 className="text-2xl font-semibold text-center text-indigo-900">
        Berikut adalah feedback interview anda dari AI
        {/* Here is your interview feedback from AI */}
      </h2>
      <h2 className=" text-indigo-800 py-2 text-xl font-semibold flex items-center gap-2">
        Overall Rating <span className="ml-10">:</span>
        <StarRating rating={averageRating(feedbackList)} />
      </h2>

      <h2 className="text-xl font-semibold text-indigo-800 mb-2">
        Overall Feedback <span className="ml-4">:</span>
      </h2>
      {overallFeedbackList &&
        overallFeedbackList.map((item, index) => (
          <h2
            className=" border rounded-lg bg-blue-100 mb-5 p-2 text-blue-900 font-semibold text-lg"
            key={index}>
      ...
      <h2 className="mt-2 text-slate-900 text-2xl font-semibold">
        Dapatkan pertanyaan dan jawaban serta feedback dari AI terhadap jawaban
        Anda
        {/* Find below interview question with correct answer, Your answer and
        detail feedback for better improvement */}
      </h2>
      {feedbackList &&
